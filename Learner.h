...
public:
    ...
    SGDLearner(loss_grad_type const& loss_grad, output_grad_type const& output_grad, loss_op_type const& loss_op, infer_op_type const& infer_op, double lr=0.001):
...
public:
    ...
    std::shared_ptr<Learner> create(LEARNER_t learner, OUTPUT_t output, double lr=0.001)
    {
        auto infer_op = std::bind(_fm_infer, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, output);
        if (learner == SGD){
            if (output == LINER){
                return std::make_shared<SGDLearner>(mse_grad, liner_grad, mse_op, infer_op, lr);
            }else if (output == SIGMOID){
                return std::make_shared<SGDLearner>(logloss_grad, sigmoid_grad, logloss_op, infer_op, lr);
            }else{
                return nullptr;
            }
        }else{
